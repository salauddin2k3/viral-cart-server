...
function eventFor(subPath, status) {
...
function actorResolvablePath(subPath) {
    return subPath === '/sign-out' || subPath === '/change-password';
}
export function authAuditLogger(req, res, next) {
    ...
    res.on('finish', () => {
        ...
        void prisma.activityLog
            .create({
            data: {
                actorId: res.locals.staffUserId ?? null,
                actorRole: res.locals.staffRole ?? null,
                action: event,
                entityType: 'auth',
...
//# sourceMappingURL=authAuditLogger.js.map