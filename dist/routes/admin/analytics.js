...
import { requireAuth, requirePermission, hasPermission } from '../../lib/guard.js';
const router = Router();
...
async function buildDashboardAnalytics(period) {
    const since = getPeriodDate(period);
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const [totalOrders, todayOrders, pendingOrders, deliveredOrders, cancelledOrders, returnedOrders, totalProducts, activeProducts, totalCategories, revenueAgg, todayRevenueAgg,] = await Promise.all([
        prisma.order.count(),
  ...
  `;
    ...
    const topProductsWithCategory = await Promise.all(topProducts.map(async (tp) => {
...
async function buildModeratorAnalytics(staff, period) {
    const since = getPeriodDate(period);
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    ...
    const result = {};
    if (canOrders) {
        const [totalOrders, todayOrders, pendingOrders, deliveredOrders, cancelledOrders, returnedOrders, revenueAgg, todayRevenueAgg, ordersByStatus, revenueByDayRows, recentOrders,] = await Promise.all([
            prisma.order.count(),
            prisma.order.count({ where: { createdAt: { gte: todayStart } } }),
            prisma.order.count({ where: { status: 'Pending' } }),
            prisma.order.count({ where: { status: 'Delivered' } }),
    ...
    if (canProducts) {
        ...
        result.summary.totalProducts = totalProducts;
        result.summary.activeProducts = activeProducts;
    }
    if (canCategories) {
        const categoryPerformanceRows = await prisma.$queryRaw `
      SELECT
...
router.get('/', async (req, res, next) => {
    try {
        ...
        const validPeriod = VALID_PERIODS.includes(period)
            ? period
            : '30d';
