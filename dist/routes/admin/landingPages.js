...
router.get('/', async (req, res, next) => {
    try {
        ...
        const take = limit + 1;
        const filterStatus = req.query.status || undefined;
        const where = {};
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        if (!page)
            throw new AppError('NOT_FOUND', 'Landing page not found');
        res.json(page);
    }
    catch (err) {
        next(err);
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const slug = ensureUniqueSlug(parsed.slug && parsed.slug.length > 0 ? parsed.slug : generateSlug(parsed.title), existingSlugs);
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        if (parsed.subtitle !== undefined)
            updateData.subtitle = parsed.subtitle || null;
        if (parsed.productId !== undefined)
            updateData.productId = parsed.productId || null;
        if (parsed.template !== undefined)
            updateData.template = parsed.template;
        if (parsed.heroImageUrl !== undefined)
            updateData.heroImageUrl = parsed.heroImageUrl || null;
        ...
        if (parsed.content !== undefined || parsed.sections !== undefined) {
            const mergedContent = {
                ...
                sections: parsed.sections ?? (existing.content?.sections ?? []),
            };
