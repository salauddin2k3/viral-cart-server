...
function isAbandoned(session: { phone: string | null; status: string; updatedAt: Date; convertedOrderId: string | null }, windowMs: number): boolean {
  if (!session.phone) return false;
  if (session.status !== 'active') return false;
  if (session.convertedOrderId) return false;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (status) {
      where.status = status as Prisma.EnumCheckoutSessionStatusFilter['equals'];
    } else {
    ...
    const order = await prisma.$transaction(async (tx) => {
      ...
      for (const item of session.items) {
        ...
        if (item.productId && product) {
          const serverFinalPrice = calculateFinalPrice(
