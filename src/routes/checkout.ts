...
router.post('/', rateLimitRegistry.autosave, async (req, res, next) => {
  try {
    ...
    if (parsed.name !== undefined) updateData.name = stripHtml(parsed.name);
    if (normalizedPhone !== null) updateData.phone = normalizedPhone;
    if (parsed.districtArea !== undefined) updateData.districtArea = stripHtml(parsed.districtArea);
    if (trimmedAddress !== undefined) updateData.address = stripHtml(trimmedAddress);
    if (parsed.note !== undefined) updateData.note = stripHtml(parsed.note);
    if (parsed.deliveryMethod !== undefined) updateData.deliveryMethod = parsed.deliveryMethod;
