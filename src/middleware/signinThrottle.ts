...
function keyFor(email: unknown, ip: string | undefined): string {
  const emailHash = createHash('sha256').update(String(email ?? '').toLowerCase()).digest('hex');
  return `${emailHash}|${ip ?? 'unknown'}`;
}

export function signInThrottle(req: Request, res: Response, next: NextFunction): void {
  ...
  res.on('finish', () => {
    const settledKey = res.locals.throttleKey as string | undefined;
    if (!settledKey) {
      return;
    }
